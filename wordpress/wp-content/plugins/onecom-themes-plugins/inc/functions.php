...
if( ! function_exists( 'onecom_themes_cat_count' ) ) {
	function onecom_themes_cat_count() {
		...
		if (is_array($themes)){
			$old_themes = array('the-anderson-family', 'summertime-adventure', 'school-days', 'personal-cv', 'heisengard', 'gardener');
			foreach($themes as  $theme_group){
				if (isset($theme_group->collection)){
					...
					foreach ($theme_group->collection as $theme){
						...
						// Skip count if old-6 themes
						if (!in_array($theme->slug, $old_themes)) {
							foreach ($theme->tags as $categories_name)

							// Increase category count if already initiated, else initiate 1
...
if( ! function_exists( 'onecom_http_requests_filter' ) ) {
	function onecom_http_requests_filter( $allow, $host, $url ) {
		...
		// allow marketgoo plugin download http_request
		$mg_info = new Onecom_Marketgoo();
		$mg_api_host = $mg_info->is_root_domain();

        $urlParts = parse_url( $check_host );
        $check_host = preg_replace('/^www\./', '', $urlParts[ 'host' ]);

        if ( $host === $check_host || $host === $mg_api_host) {
            $allow = true;
