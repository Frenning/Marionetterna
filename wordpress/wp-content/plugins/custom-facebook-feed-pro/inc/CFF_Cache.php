...
class CFF_Cache {
	...
	public function retrieve_and_set() {
		...
		if ( ! $this->is_legacy ) {
			...
			$transient_cache = $this->encryption->maybe_decrypt(get_transient( $this->feed_id ));
			if ( $transient_cache ) {
			...
			$this->posts_backup = $this->encryption->maybe_decrypt(get_transient( '!cff_backup_' . $this->feed_id ));
		}
