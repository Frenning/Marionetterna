...
if (!class_exists('OneComCentralizedMenu')) {
    require dirname(__FILE__).'/inc/lib/class-onecom-centralized-menu.php';
...
final class OCVCaching extends VCachingOC {
    ...
    public function __construct() {
        ...
        add_action('wp_ajax_oc_set_dev_mode_time', array($this, 'oc_set_dev_mode_time_cb'));
        add_action('wp_ajax_oc_exclude_cdn_mode', array($this, 'oc_exclude_cdn_mode_cb'));
        add_action('wp_ajax_oc_exclude_cdn_data', array($this, 'oc_exclude_cdn_data_cb'));
    ...
    */
    public function runAdminSettings() {
        ...
        // @todo - to local menu display, remove before git push
    ...
    **/
    public function onActivatePlugin() {
        ...
        if($this->oc_premium() === true || $this->oc_pcache() === true) { 
            $cdn_enabled = update_site_option(self::optionCDN, "true");
            self::setDefaultSettings();
        } else {
            $cdn_disabled = update_site_option(self::optionCDN, "false");
            $ttl_disabled = update_option( self::defaultPrefix . 'enable', "false" );
        }
...
//
        ...
        if($this->oc_pcache() === false && $this->oc_premium() === false) {
    ...
    */
    ...
    public function oc_set_dev_mode_cb(){
        ...
        if ($this->oc_json_get_option('onecom_vcache_info','oc_dev_mode_enabled') == $state){
            ...
            $result = $this->oc_json_update_option('onecom_vcache_info', $dev_mode_state);
            
            $dev_duration_data = array('dev_mode_duration'=> $dev_mode_duration);
            $result = $this->oc_json_update_option('onecom_vcache_info', $dev_duration_data);
    ...
    // Update 'development mode' duration
    public function oc_set_dev_mode_time_cb($echo){
        ...
        $dev_mode_duration === 0 ? 48 : $dev_mode_duration ;
        /* if ((get_site_option('dev_mode_duration') == $dev_duration)){
            $result = true;
        }else{ */
            $dev_expire_time = strtotime("+$dev_mode_duration hours");
            
            $dev_mode_duration_data = array('dev_mode_duration'=> $dev_mode_duration);
            $result = $this->oc_json_update_option('onecom_vcache_info',$dev_mode_duration_data);
            $dev_expire_time_data = array('dev_expire_time'=> $dev_expire_time);
            $result = $this->oc_json_update_option('onecom_vcache_info',$dev_expire_time_data);
    ...
    // Update 'Exclude CDN' data/rules
    public function oc_exclude_cdn_data_cb($echo){
    ...
    public function rewrite($html){
        ...
        //$pattern = "/(?:https:\/\/$domain_name\/wp-content)(\S*\.(?:[0-9a-zA-Z]+))\b/m";
        // $pattern = "/(?:https:\/\/$domain_name\/wp-content)(\S*\.[0-9a-zA-Z]+)\b/m"; 
        //$pattern = '#(?<=[(\"\'])' . $url . '/(?:((?:'.$directories.')[^\"\')]+)|([^/\"\']+\.[^/\"\')]+))(?=[\"\')])#i';
    ...
    protected function rewrite_asset_url($asset) {
        ...
        */
        ...
        if ( $preview_condition || $path_condition || $extension_condition || $staging_condition || $template_path_condition)
        {
            return $asset[0];
        }
